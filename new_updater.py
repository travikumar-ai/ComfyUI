...
def update_windows_updater():
    ...
    if not contents.startswith(b"..venv\\Scripts\\python.exe .\\update.py"):
        return
    ...
    try:
        with open(dest_bat_deps_path, 'rb') as f:
            contents = f.read()
            contents = contents.replace(b'..venv\\Scripts\\python.exe .\\update.py ..\\ComfyUI\\', b'call update_comfyui.bat nopause')
